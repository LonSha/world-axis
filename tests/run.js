...
// ══════════════════════════════════════════════════════════════════
  ...
  section('v2.27.0：死子面冻结账本（dead/uiDead 不再是一条静默增长的面）');
  {
    ...
    assert(r2700.refs === 1386, '现场静态引用 1386 处（真代码口径，实 ' + r2700.refs + '）');
    assert(r2700.namespaces === 66 && r2700.members === 707,
      '定义面 66 命名空间 / 707 成员（实 ' + r2700.namespaces + '/' + r2700.members + '）');
    ...
    console.log('  ✓ 现场锚点（refs 1386 / 命名空间 66 / 成员 707 / dead 208 · uiDead 4 · dataOnly 106）');
  ...
  section('v2.28.0：账本元数据与归因证据强度（把「归因可读」推到「归因可证伪」）');
  {
    ...
    assert(r2800.refs === 1386 && r2800.namespaces === 66 && r2800.members === 707,
      '清册面（refs 1386 / 命名空间 66 / 成员 707，真代码口径）');
    ...
    console.log('  ✓ 归因带证（215 条全带 src/refs/tref/own，归因由证据唯一反推）');
  ...
  // ══════════ v2.29.0 ══════════
  section('v2.29.0：引用面的输入面必须是真代码面（提及不是引用）');
  {
    // 命题：v2.28.0 让归因**可证伪**（每条带 src/refs/tref/own 并可复算），但「refs 到底该算谁」
    //   这件事本身从没被审过。交付时现场有两处铁证：
    //   ① 在任一产品文件末尾加**一行纯注释**（零代码改动）⇒ 冻结面 208→207，该成员脱出死子面，
    //      门禁输出「⚠ 已登记的死导出消失 1 项」+「✓」并 **exit 0 绿灯**，还催人跑 --update——
    //      照办就把该条目从账本**永久删除**。即：**死子面被一行注释掏空**，比 v2.27.0 治的
    //      「新增无人提示」更隐蔽（新增会红灯，掏空只提示）。
    //   ② 门禁自写的 stripNonCode 不识别正则字面量：字符类正则里的引号让状态机进字符串态，
    //      而单双引号串在旧实现里**不因换行终止** ⇒ **跨行失步**，把其后整段真代码剥成空格。
    //      实测丢失真代码引用 97 处（ui/panel.js 58 / ui/settings.js 38 / engines/tool-snapshot.js 1）。
    // 修法（单源）：引用面的**输入**统一为**真代码面**——`tests/inventory.js` 导出 codeFace(src)，
    //   门禁不再自写剥离器（旧 stripNonCode 已删除并移出导出面）。清册与门禁从此只有一个口径。
    const inv2900 = require('./inventory.js');
    const gate2900 = require('./dead-export-gate.js');
    const invSrc2900 = fs.readFileSync(path.join(BASE, 'tests/inventory.js'), 'utf8');
    const gsrc2900 = fs.readFileSync(path.join(BASE, 'tests/dead-export-gate.js'), 'utf8');
    const r2900 = inv2900.collect();
    const led2900 = gate2900.loadLedger();
    const clone2900 = function (o) { return JSON.parse(JSON.stringify(o)); };
    const md5of2900 = function (s) { return require('crypto').createHash('md5').update(s).digest('hex'); };

    // ── A. 结构锁：真代码面只有一个来源 ──
    assert(typeof inv2900.codeFace === 'function' && inv2900.codeFace.length === 1,
      '清册导出 codeFace(src)（真代码面提取器，单源）');
    assert(gate2900.stripNonCode === undefined, '门禁导出面已无自写剥离器 stripNonCode（不再各写一份口径）');
    assert(gsrc2900.indexOf('function stripNonCode') < 0, '[静态] 门禁源码内不再有 stripNonCode 定义');
    assert(gsrc2900.indexOf('inventory.codeFace') > 0 && invSrc2900.indexOf('codeFace: codeFace') > 0,
      '[静态] 门禁的引用计数走清册 codeFace（口径只有一个来源）');
    assert(gate2900.judge(r2900, led2900).ok === true, '（基线）现场账本 ⇒ ok');

    // ── B. codeFace 守恒性（长度 / 行数）与保真性（该留的留、该剥的剥）──
    const lenBad2900 = [], lineBad2900 = [];
    inv2900.PRODUCT_FILES.forEach(function (rel) {
      const src = fs.readFileSync(path.join(BASE, rel), 'utf8');
      const out = inv2900.codeFace(src);
      if (out.length !== src.length) lenBad2900.push(rel);
      if (out.split('\n').length !== src.split('\n').length) lineBad2900.push(rel);
    });
    assert(lenBad2900.length === 0 && lineBad2900.length === 0,
      'codeFace 在 ' + inv2900.PRODUCT_FILES.length + ' 个产品文件上长度守恒 + 行数守恒（原位空格替换，行号仍可与原文对照）');
    const mini2900 = 'const a = /[&<>"]/g;\nconst b = WA.foo.bar();\nconst c = WA.baz.qux;\n';
    const face2900 = inv2900.codeFace(mini2900);
    assert(face2900.length === mini2900.length && face2900.split('\n').length === mini2900.split('\n').length,
      '（最小复现）含字符类正则的样本守恒：长度 ' + face2900.length + ' / 原文 ' + mini2900.length);
    assert(face2900.indexOf('WA.foo.bar') > 0 && face2900.indexOf('WA.baz.qux') > 0,
      '（最小复现）正则字面量不吞后文——其后两行真代码仍在真代码面上（旧实现此处全丢）');
    const mix2900 = "// WA.cmt.mem 已接通\nconst s = 'WA.str.mem';\nconst t = `tpl ${WA.tpl.mem} tail`;\nconst u = \"WA.dq.mem\";\n";
    const fm2900 = inv2900.codeFace(mix2900);
    assert(fm2900.indexOf('WA.cmt.mem') < 0 && fm2900.indexOf('WA.str.mem') < 0 && fm2900.indexOf('WA.dq.mem') < 0,
      '（保真）行注释 / 单引号串 / 双引号串里的成员提及全部剥掉——提及不是引用');
    assert(fm2900.indexOf('WA.tpl.mem') > 0, '（保真）模板字符串 ${} 插值内的表达式是真代码，不剥');
    assert(inv2900.codeFace('const p = `WA.plain.mem`;\n').indexOf('WA.plain.mem') < 0,
      '（保真）模板字符串的纯文本部分只是一段字，剥掉');

    // ── C. 本轮缺陷的直接锁：加注释不得改变冻结面 ──
    const srcBridge2900 = fs.readFileSync(path.join(BASE, 'engines/bridge.js'), 'utf8');
    const cmtAdd2900 = srcBridge2900 + '\n// 修：外部调用方可直接用 WA.bridge.snapshot 读快照\n';
    const callAdd2900 = srcBridge2900 + '\nfunction __ncProbeBridgeSnapshot() { return WA.bridge.snapshot; }\n';
    const refCmt2900 = gate2900.refCountIn(cmtAdd2900)['bridge.snapshot'] || 0;
    const refCall2900 = gate2900.refCountIn(callAdd2900)['bridge.snapshot'] || 0;
    assert(refCmt2900 === 0, '（负控制）加一行注释后 bridge.snapshot 的产品引用数仍为 0（实 ' + refCmt2900 + '）——提及不是引用');
    assert(refCall2900 > 0, '（负控制对照）真写一次调用 ⇒ 计为引用（实 ' + refCall2900 + '）——真消费方才可解冻');

    // ── D. 端到端真源码注入：注释注入 ⇒ 门禁结论一字不变；真调用注入 ⇒ 报「死导出消失」 ──
    const pBridge2900 = path.join(BASE, 'engines/bridge.js');
    const bakBridge2900 = fs.readFileSync(pBridge2900, 'utf8');
    ...
    try {
      vBase2900 = runGate2900();
      fs.writeFileSync(pBridge2900, cmtAdd2900);
      vCmt2900 = runGate2900();
      fs.writeFileSync(pBridge2900, callAdd2900);
      vCall2900 = runGate2900();
    } finally {
      fs.writeFileSync(pBridge2900, bakBridge2900);
      md5After2900 = md5of2900(fs.readFileSync(pBridge2900, 'utf8'));
    }
    assert(md5After2900 === md5Bridge2900, '（负控制）注入结束已逐字节还原 engines/bridge.js（md5 ' + md5After2900.slice(0, 8) + '）');
    assert(vBase2900 && vBase2900.ok === true && vBase2900.gone.length === 0, '（基线）门禁绿、无消失条目');
    assert(vCmt2900 && vCmt2900.ok === true && vCmt2900.gone.length === 0 && vCmt2900.added.length === 0,
      '（铁证一锁）加一行纯注释 ⇒ 冻结面一字不变（旧口径下此处会报「bridge.snapshot 消失」并催跑 --update ⇒ 条目被永久删除）');
    assert(vCall2900 && vCall2900.ok === true && vCall2900.added.length === 0
      && vCall2900.gone.length === 1 && vCall2900.gone[0].key === 'bridge.snapshot',
      '（铁证一对照）真写一次调用 ⇒ 报「已登记的死导出消失 1 项：bridge.snapshot」（真消费方被识别出来）');

    // ── E. 异常隔离：复算失败 ⇒ 记一条失实，不许静默跳过、不许抛栈中断 ──
    const ANCH_EV2900 = 'function evidenceOf(rec) {';
    const cntEv2900 = gsrc2900.split(ANCH_EV2900).length - 1;
    assert(cntEv2900 === 1, '（负向自证）证据计算锚点在真源码中恰 1 处（实 ' + cntEv2900 + '）');
    const brokenEv2900 = gsrc2900.replace(ANCH_EV2900, ANCH_EV2900 + "\n  throw new Error('NC-复算故障');");
    assert(brokenEv2900 !== gsrc2900, '（负向自证）破坏真的改变了源码');
    const tmpEv2900 = path.join(__dirname, '_nc_gate_ev_break.js');
    let bEv2900 = null, driftBoom2900 = null, driftErr2900 = null;
    try {
      fs.writeFileSync(tmpEv2900, brokenEv2900);
      delete require.cache[require.resolve(tmpEv2900)];
      bEv2900 = require(tmpEv2900);
      try { driftBoom2900 = bEv2900.evidenceDrift(r2900, led2900); } catch (e) { driftErr2900 = e; }
    } finally {
      // 只 require、不跑 CLI（main 不执行 ⇒ 绝不触碰真实账本）
      try { fs.unlinkSync(tmpEv2900); } catch (e) {}
      try { delete require.cache[require.resolve(tmpEv2900)]; } catch (e) {}
    }
    assert(driftErr2900 === null, '（负控制）复算故障不抛栈中断（算不出来不得把整条链带崩）'
      + (driftErr2900 ? ' —— 却抛了 ' + driftErr2900.message : ''));
    assert(Array.isArray(driftBoom2900) && driftBoom2900.length > 0
      && driftBoom2900.every(function (x) { return x.field === '（复算）' && x.detail.indexOf('证据复算失败') === 0; }),
      '（负控制）复算故障逐条记为「证据复算失败——算不出来不得当作算过」（' + (driftBoom2900 ? driftBoom2900.length : '?') + ' 条）');
    ...
    assert(r2900.refs === 1386 && r2900.namespaces === 66 && r2900.members === 707,
      '清册面（refs 1386 / 命名空间 66 / 成员 707）——真代码口径下的现场值');
    ...
    assert(soft2900.every(function (k) { return ledKeys2900.indexOf(k) >= 0; }),
      '（口径升级）只被注释/字符串提及的 ' + soft2900.join(' / ') + ' 已进入冻结面（旧口径把「提及」算成「活着」）');
    const softSet2900 = {};
    soft2900.forEach(function (k) { softSet2900[k] = true; });
    let rawHit2900 = 0, codeHit2900 = 0;
    inv2900.PRODUCT_FILES.forEach(function (rel) {
      const src = fs.readFileSync(path.join(BASE, rel), 'utf8');
      [src, inv2900.codeFace(src)].forEach(function (text, idx) {
        const re = new RegExp(inv2900.REF_RE.source, 'g');
        let m;
        while ((m = re.exec(text))) {
          if (!softSet2900[m[1] + '.' + m[2]]) continue;
          if (idx === 0) rawHit2900 += 1; else codeHit2900 += 1;
        }
      });
    });
    assert(codeHit2900 === 0 && rawHit2900 > 0,
      '（铁证）这三个成员的提及：原文命中 ' + rawHit2900 + ' 次、真代码面命中 ' + codeHit2900 + ' 次——旧口径把「提及」当「引用」');
    // 冻结面、advisory 面与判据宽度本版不扩不改
    assertDeepEq(gate2900.FROZEN_KINDS, ['dead', 'uiDead'], '冻结面仍为 dead/uiDead（本版只换输入面、不扩面）');
    assertDeepEq(gate2900.ADVISORY_KINDS, ['dataOnly'], 'advisory 面仍为 dataOnly（不升级为拦截）');
    ...
    assert(gate2900.judge(r2900, ledAdd2900).added.length === 1, '新增死导出仍红灯（旧判据未被替换）');
    const ledCorrupt2900 = clone2900(led2900);
    ledCorrupt2900.dead[Object.keys(ledCorrupt2900.dead)[0]].reason = 'TODO';
    assert(gate2900.judge(r2900, ledCorrupt2900).staleReason.length === 1, '归因不可读仍红灯（旧判据未被替换）');
    assert(gate2900.judge(r2900, null).ok === false, '账本缺失仍红灯（旧判据未被替换）');
    const ledMF2900 = clone2900(led2900); ledMF2900.version = '2.28.0';
    assert(gate2900.judge(r2900, ledMF2900).ok === false, '（旧灯）元数据不三级同源仍红灯');
    const ledNoEv2900 = clone2900(led2900);
    ledNoEv2900.dead[Object.keys(ledNoEv2900.dead)[0]] = { reason: 'unwired', detail: 'x' };
    assert(gate2900.judge(r2900, ledNoEv2900).evidence.length > 0, '（旧灯）账本留无证记录仍红灯');

    // ── G. 负向自证：拆掉真代码面的两条识别规则 ⇒ 探测器失步（跨行把真代码剥成空格）──
    const ANCH_Q2900 = "if (c === '\\n') { stack.pop(); out += '\\n'; i += 1; continue; }";
    const cntQ2900 = invSrc2900.split(ANCH_Q2900).length - 1;
    assert(cntQ2900 === 1, '（负向自证）引号串换行终止锚点在真源码中恰 1 处（实 ' + cntQ2900 + '）');
    const brokenQ2900 = invSrc2900.replace(ANCH_Q2900, "if (false) { stack.pop(); out += ' '; i += 1; continue; }");
    assert(brokenQ2900 !== invSrc2900, '（负向自证）破坏真的改变了源码');
    const ANCH_RE2900 = "if (prev === '' || CONT.indexOf(prev) >= 0) {";
    const cntRe2900 = invSrc2900.split(ANCH_RE2900).length - 1;
    assert(cntRe2900 === 1, '（负向自证）正则字面量识别锚点在真源码中恰 1 处（实 ' + cntRe2900 + '）');
    const brokenRe2900 = invSrc2900.replace(ANCH_RE2900, 'if (false) {');
    assert(brokenRe2900 !== invSrc2900, '（负向自证）破坏真的改变了源码');
    const sampleQ2900 = 'const a = "unclosed\nconst b = WA.foo.bar();\nconst c = WA.baz.qux;\n';
    const sampleRe2900 = "const a = /['\"]/g; const b = WA.foo.bar();\nconst c = WA.baz.qux;\n";
    const tmpQ2900 = path.join(__dirname, '_nc_inv_q_break.js');
    const tmpRe2900 = path.join(__dirname, '_nc_inv_re_break.js');
    let faceQ2900 = '', faceRe2900 = '', refsQ2900 = null;
    try {
      fs.writeFileSync(tmpQ2900, brokenQ2900);
      delete require.cache[require.resolve(tmpQ2900)];
      const bQ2900 = require(tmpQ2900);
      faceQ2900 = bQ2900.codeFace(sampleQ2900);
      refsQ2900 = bQ2900.collect().refs;
      fs.writeFileSync(tmpRe2900, brokenRe2900);
      delete require.cache[require.resolve(tmpRe2900)];
      const bRe2900 = require(tmpRe2900);
      faceRe2900 = bRe2900.codeFace(sampleRe2900);
    } finally {
      try { fs.unlinkSync(tmpQ2900); } catch (e) {}
      try { fs.unlinkSync(tmpRe2900); } catch (e) {}
      try { delete require.cache[require.resolve(tmpQ2900)]; } catch (e) {}
      try { delete require.cache[require.resolve(tmpRe2900)]; } catch (e) {}
    }
    assert(faceQ2900.indexOf('WA.foo.bar') < 0 && faceQ2900.indexOf('WA.baz.qux') < 0,
      '（负向自证）拆掉「引号串遇换行终止」⇒ 跨行失步：其后两行真代码全被剥成空格');
    assert(faceQ2900.length === sampleQ2900.length,
      '（负向自证）失步表现为「剥成空格」而非「删字符」——长度仍守恒，只能靠内容判据发现');
    assert(refsQ2900 !== r2900.refs,
      '（负向自证）破坏版全仓引用面随之偏移（' + refsQ2900 + ' ≠ ' + r2900.refs + '）——症状可观测，不是只坏一个辅助函数');
    assert(faceRe2900.indexOf('WA.foo.bar') < 0 && faceRe2900.indexOf('WA.baz.qux') > 0,
      '（负向自证）拆掉「正则字面量识别」⇒ 正则被当除法、其内引号进字符串态吞掉同行后半与下一行（更远一行恢复）');
    assert(inv2900.codeFace(sampleQ2900).indexOf('WA.foo.bar') > 0
      && inv2900.codeFace(sampleRe2900).indexOf('WA.foo.bar') > 0,
      '（负向自证）同一输入在原版上两行都保真（判据纯度）');

    console.log('  ✓ 真代码面单源（长度/行数守恒，注释与字符串剥掉、${} 内保留）');
    console.log('  ✓ 注释不解冻（加一行注释冻结面一字不变；真写调用才报「死导出消失」）');
    console.log('  ✓ 异常隔离（复算失败逐条记账，不静默跳过、不抛栈中断）');
    console.log('  ✓ 口径升级（dead 208→211 的差量恰是旧口径算作活着的「提及」）');
    console.log('  ✓ 负向自证（拆掉真代码面识别规则 ⇒ 跨行失步重现、原版同输入必保真）');
  }
  ...
  // ══════════ v2.36.0 ══════════
  // 第二十三面：轮次单一真源。全库  有 6 处读者、零写入方——真源是 evolution.round。
  //   本段锁四件事：真源优先级、账本/纪事/摘要三处入账轮次、同轮重 roll 覆盖恢复有效、静态口径只留兼容读一处。
  section('v2.36.0：轮次单一真源（evolution.roundOf）');
  {
    const W = WA;
    const setRound = function (r) { W.store.transact(function (d) { d.evolution.round = r; }); };
    const setMetaRound = function (r) { W.store.transact(function (d) { d.meta = d.meta || {}; d.meta.round = r; }); };
    const clrMetaRound = function () { W.store.transact(function (d) { if (d.meta) delete d.meta.round; }); };
    // ── A. 真源优先级：evolution.round 胜出，meta.round 只作历史兼容兜底 ──
    assert(typeof W.evolution.roundOf === 'function', 'v2360: evolution.roundOf 对外可读（唯一轮次读口）');
    clrMetaRound(); setRound(7);
    assert(W.evolution.roundOf() === 7, 'v2360: 真源 evolution.round 生效（实 ' + W.evolution.roundOf() + '）');
    setMetaRound(3);
    assert(W.evolution.roundOf() === 7, 'v2360: （负向）手写 meta.round=3 不得压过真源（实 ' + W.evolution.roundOf() + '）');
    W.store.transact(function (d) { delete d.evolution.round; });
    assert(W.evolution.roundOf() === 3, 'v2360: 真源缺失时兼容读旧 meta.round（历史存档不丢轮次，实 ' + W.evolution.roundOf() + '）');
    clrMetaRound();
    assert(W.evolution.roundOf() === 0, 'v2360: 两处都缺 ⇒ 0（与旧行为一致，不抛）');
    assert(W.evolution.roundOf({}) === 0, 'v2360: 传入空壳状态不抛（读失败按 0）');
    // ── B. 账本按真源入账（旧实现读 meta.round ⇒ 永远「第0轮」）──
    W.store.transact(function (d) {
      d.evolution.round = 7;
      d.meta = d.meta || {};
      d.meta.round = 99;
      d.evolution.events = [{ id: 'e2360a', title: 'v2360盐船失事', type: 'conflict', level: 3, stage: '发酵' }];
      d.evolution.winds = [];
      d.evolution.ledger = [];
    });
    W.ledger.saveCheckpoint();
    W.store.transact(function (d) {
      d.evolution.events[0].stage = '已爆发';
      d.evolution.winds.push({ id: 'w2360', topic: 'v2360血月', level: 3, content: '血月将现' });
    });
    W.ledger.recordChanges();
    let led2360 = W.store.get().evolution.ledger;
    assert(led2360.length === 1, 'v2360: 账本入账一条（实 ' + led2360.length + '）');
    assert(led2360[0].round === 7, 'v2360: 账本轮次取真源 7（实 ' + led2360[0].round + '，旧实现恒 0）');
    assert(W.ledger.buildLedgerText().indexOf('第7轮') >= 0, 'v2360: 注入文本显第7轮');
    assert(W.ledger.buildLedgerText().indexOf('第99轮') < 0, 'v2360: （负向）meta.round=99 不得进注入文本');
    // ── C. 同轮重 roll 覆盖恢复有效（旧实现 round 恒 0 ⇒ 判据恒真，每轮新压一条）──
    W.ledger.saveCheckpoint();
    W.store.transact(function (d) { d.evolution.winds.push({ id: 'w2360b', topic: 'v2360第二条', level: 3, content: 'x' }); });
    W.ledger.recordChanges();
    led2360 = W.store.get().evolution.ledger;
    assert(led2360.length === 1, 'v2360: 同轮重 roll 覆盖不堆积（实 ' + led2360.length + ' 条，旧实现此处 2 条）');
    setRound(8);
    W.ledger.saveCheckpoint();
    W.store.transact(function (d) { d.evolution.winds.push({ id: 'w2360c', topic: 'v2360第三条', level: 3, content: 'y' }); });
    W.ledger.recordChanges();
    led2360 = W.store.get().evolution.ledger;
    assert(led2360.length === 2 && led2360[0].round === 8 && led2360[1].round === 7,
      'v2360: 跨轮各留一条且轮次可辨（实 ' + led2360.map(function (m) { return m.round; }).join('/') + '）');
    // ── D. 纪事（horizon）与摘要（digest）轮次同源 ──
    W.store.transact(function (d) { d.evolution.round = 5; d.meta = d.meta || {}; d.meta.round = 99; d.chronicle = []; });
    assert(W.horizon.acceptResult('distant', { title: 'v2360远方事', desc: '边镇戒严' }) === true, 'v2360: horizon 远方入账成功');
    assert((W.store.get().chronicle || []).slice(-1)[0].round === 5,
      'v2360: 纪事轮次取真源 5（实 ' + ((W.store.get().chronicle || []).slice(-1)[0] || {}).round + '，旧实现恒 0）');
    setRound(5);
    W.digest.generate();
    assert(W.store.get().evolution.worldDigest.round === 5,
      'v2360: 世界摘要轮次取真源 5（实 ' + W.store.get().evolution.worldDigest.round + '）');
    setRound(6);
    assert(W.horizon.acceptResult('near', { title: 'v2360近端事', description: '城门戒严' }) === true, 'v2360: horizon 近端入账成功');
    assert((W.store.get().chronicle || []).slice(-1)[0].round === 6, 'v2360: 近端纪事轮次随真源推进到 6');
    // ── E. 静态口径：产品代码只剩 evolution.js 一处兼容读，别处一律走读口 ──
    const inv2360 = require('./inventory.js');
    const prod2360 = inv2360.PRODUCT_FILES || [];
    let mrHits2360 = 0;
    const mrFiles2360 = [];
    prod2360.forEach(function (rel) {
      const face = inv2360.codeFace(fs.readFileSync(path.join(BASE, rel), 'utf8'));
      const n = (face.match(/meta *[.] *round/g) || []).length;
      if (n) { mrHits2360 += n; mrFiles2360.push(rel + ':' + n); }
    });
    assert(mrHits2360 === 2 && mrFiles2360.length === 1 && mrFiles2360[0] === 'engines/evolution.js:2',
      'v2360: 全库 meta.round 真代码引用只剩 evolution.js 兼容读 2 处（实 ' + mrHits2360 + ' @ ' + mrFiles2360.join(',') + '）');
    console.log('  ✓ v2360: 轮次真源优先级 + 兼容兜底 + 不抛');
    console.log('  ✓ v2360: 账本/纪事/摘要三处入账轮次同源 + 同轮覆盖恢复 + 静态口径收口');
  }

  // ══════════ v2.37.0 ══════════
  // 第二十四面：实体库闭环。buildEntitiesBlock 自述「推演必须复用、不得重复造同义实体」，
  //   但全库唯一调用方是 tool-analyzer（诊断）与测试——backstage 只写不读，机制是死的。
  //   本段锁三件事：提示词真含既有实体、空库不产垃圾段、结算侧写得进（写读成对）。
  section('v2.37.0：既有实体清单进推演提示词（实体库闭环）');
  {
    const W = WA;
    const userOf = function () {
      const msgs = W.backstage.buildPrompt({ idx: 1, text: "x" }, "");
      const u = msgs.filter(function (m) { return m.role === "user"; })[0];
      return u ? u.content : "";
    };
    // 空库：不得注入空头段（宁缺毋滥，与全库口径一致）
    W.store.transact(function (d) { d.evolution.entityMemory = { organization: [], object: [], ability: [], location: [] }; });
    assert(W.entities.buildEntitiesBlock() === "", "v2370: 空实体库 buildEntitiesBlock 返回空串");
    assert(userOf().indexOf("既有实体库") < 0, "v2370: 空库时推演提示词不出现空头段");
    // 写入：结算侧 upsert 真进 state
    const ru = W.store.transact(function (d) { return W.entities.upsert(d, "organization", { name: "v2370盐帮", aliases: ["淮北盐帮"], desc: "运河漕运巨擘" }); });
    assert(ru.ok && ru.result === "created", "v2370: entities.upsert 真写入新建");
    const ru2 = W.store.transact(function (d) { return W.entities.upsert(d, "location", { name: "v2370盐码头", desc: "夜半起火处" }); });
    assert(ru2.ok && ru2.result === "created", "v2370: 第二类实体写入");
    // 读取：提示词真含实体（本轮修复的判据）
    const blk = W.entities.buildEntitiesBlock();
    assert(blk.indexOf("既有实体库") >= 0 && blk.indexOf("v2370盐帮") >= 0 && blk.indexOf("淮北盐帮") >= 0, "v2370: buildEntitiesBlock 含名与别名");
    const u = userOf();
    assert(u.indexOf("v2370盐帮") >= 0, "v2370: 推演提示词真含既有实体（旧实现查无此名）");
    assert(u.indexOf("既有实体库") >= 0, "v2370: 推演提示词含实体段头（防重复造实体的指令真到位）");
    assert(u.indexOf("v2370盐码头") >= 0, "v2370: 第二类实体同样进提示词");
    // 负向：清空后再看——不得留残影
    W.store.transact(function (d) { d.evolution.entityMemory = { organization: [], object: [], ability: [], location: [] }; });
    assert(userOf().indexOf("v2370盐帮") < 0, "v2370: （负向）实体清空后提示词不得再含其名");
    // 静态口径：backstage 必须真调 buildEntitiesBlock（防日后回退）
    const srcBs2370 = fs.readFileSync(path.join(BASE, "engines/backstage.js"), "utf8");
  ...
  // ══════════ v2.39.0 ══════════
  // 第二十六面：顶层 state.round 幽灵（v2.36.0 收口漏网）。骨架里根本没有顶层 `round` 字段，
  //   真源只有 evolution.round，但全库仍有 5 处读顶层 round / meta.round：
  //     · engines/proactive.js  —— cooldownOk 与 markPulled（冷却判据读幽灵 0）
  //     · engines/inject-inspector.js / engines/tool-diag.js —— 诊断轮次恒 null / undefined
  //     · engines/chatcache.js —— 自动备份读 meta.round ⇒ 恒 0 ⇒ 轮次推进自动备份永不触发
  //   v2.36.0 的静态锁只认字面 `meta.round`，`).meta || {}).round` 这种嵌套写法不命中 ⇒ 漏网。
  //   本段锁五件事：写入口写真源、冷却真生效（含负向）、诊断/自检同源、静态口径收口、负向自证。
  section('v2.39.0：顶层 round 幽灵收口（冷却真生效 + 自动备份/诊断同源）');
  {
    const W = WA;
    const inv2390 = require("./inventory.js");
    const ghostScan = function (face) {
      return (face.match(/(^|[^.\w$])(st|state|snap|env|d|draft|tx|o)\.round\b/g) || []).length;
    };
    // 现场基线：顶层 round 不是真源
    W.store.transact(function (d) { delete d.round; d.evolution.round = 5; });
    assert(W.store.get().round === undefined, "v2390: 现场确认顶层 state.round 不存在（真源是 evolution.round）");
    // ── A. 冷却写入口必须写真源（旧实现恒写 0）──
    W.store.transact(function (d) { delete d.proactiveLastRound; d.evolution.round = 5; });
    W.proactive.markPulled();
    assert(W.store.get().proactiveLastRound === 5,
      "v2390: markPulled 记录真源轮次（实 " + W.store.get().proactiveLastRound + "，旧实现恒 0）");
    // ── B. 冷却判据真生效：未推进 ⇒ 冷却中；推进超阈值 ⇒ 放行 ──
    const node2390 = W.workflow.list("before").filter(function (n) { return n.id === "proactive.pull"; })[0];
    assert(!!node2390, "v2390: proactive.pull 节点在册");
    const drained = function (mes) {
      return { type: "normal", chat: [{ role: "user", mes: mes }], store: W.store.get(), injections: [] };
    };
    W.store.transact(function (d) { d.evolution.round = 6; });
    const cA2390 = drained("嗯"); await node2390.run(cA2390);
    assert(cA2390.injections.length === 0, "v2390: 冷却内（6-5=1 < 3）不重复拉动");
    W.store.transact(function (d) { d.evolution.round = 20; });
    const cB2390 = drained("哦"); await node2390.run(cB2390);
    assert(cB2390.injections.length === 1,
      "v2390: 冷却到期（20-5=15 ≥ 3）真拉动（旧实现此处永久 0 条，整引擎拉一次后死）");
    assert(W.store.get().proactiveLastRound === 20,
      "v2390: 拉动后冷却计数同步真源（实 " + W.store.get().proactiveLastRound + "）");
    // ── C. 诊断包世界状态同源（旧实现 undefined）──
    W.store.transact(function (d) { d.evolution.round = 33; });
    const dg2390 = W.toolDiag.collect();
    const ws2390 = dg2390 && dg2390.worldState;
    assert(!!ws2390 && ws2390.round === 33,
      "v2390: 诊断包世界状态轮次取真源 33（实 " + (ws2390 && ws2390.round) + "，旧实现 undefined）");
    // ── D. 注入自检快照同源（旧实现 null）──
    const env2390 = W.injectInspector.snapEnv(global.SillyTavern.getContext(), {});
    assert(!!env2390 && env2390.round === 33,
      "v2390: 注入自检快照轮次取真源 33（实 " + (env2390 && env2390.round) + "，旧实现 null）");
    // ── E. 静态口径：全库顶层 .round 幽灵读点只剩合法项 ──
    //   parallel-world.js 的 st.round 是 pwState() 自产对象（自带 round 字段），属合法；
    //   ui/panel.js 读的是快照对象字段（sp.round/ls.round），不在扫描标识符集内。
    const prod2390 = inv2390.PRODUCT_FILES || [];
    const ghostFiles = [];
    prod2390.forEach(function (rel) {
      const face = inv2390.codeFace(fs.readFileSync(path.join(BASE, rel), "utf8"));
      const n = ghostScan(face);
      if (n) ghostFiles.push(rel + ":" + n);
    });
    //   合法残留仅三类：inject-inspector 的 env（snapEnv 自产快照对象）、tool-diag 的 snap
    //   （自检快照对象）、parallel-world 的 st（pwState 自产对象）——它们都自带 round 字段，
    //   不是世界状态；真正读「世界状态顶层 round」的幽灵已全部清零。
    const LEGAL_GHOST_2390 = "engines/inject-inspector.js:2,engines/parallel-world.js:1,engines/tool-diag.js:1";
    assert(ghostFiles.join(",") === LEGAL_GHOST_2390,
      "v2390: 顶层 .round 幽灵读点只剩本地构造对象三处（实 " + ghostFiles.join(",") + "，期望 " + LEGAL_GHOST_2390 + "）");
    // ── F. 负向自证：把修复退回旧写法 ⇒ 扫描器必须现形 ──
    const srcPr2390 = fs.readFileSync(path.join(BASE, "engines/proactive.js"), "utf8");
    const facePr2390 = inv2390.codeFace(srcPr2390);
    assert(ghostScan(facePr2390) === 0, "v2390: （负向自证）修复后 proactive 真代码面无幽灵读点");
    const brokenPr2390 = srcPr2390
      .split("const round = roundOfSafe(state);").join("const round = st.round != null ? st.round : 0;")
      .split("d.proactiveLastRound = roundOfSafe(d);").join("d.proactiveLastRound = d.round != null ? d.round : 0;");
    assert(brokenPr2390 !== srcPr2390, "v2390: （负向自证）破坏锚点在真源码中恰命中（可观测改行为）");
    //   本断言只扫 proactive.js 一份文件面：坏写法在文件内各出现两次
    //   （`st.round` 读 + 判空各一、`d.round` 读 + 判空各一）⇒ 合计 4 处。
    assert(ghostScan(inv2390.codeFace(brokenPr2390)) === 4,
      "v2390: （负向自证）退回旧写法后扫描器真报 4 处（st.round/d.round 各 2，实 "
      + ghostScan(inv2390.codeFace(brokenPr2390)) + "）");
    // ── G. chatcache 自动备份：轮次推进必须能产出自动快照（旧实现读 meta.round 恒 0）──
    //   该处是 `(JSON.parse(getState(id)||'{}').meta||{}).round` 嵌套写法，v2.36.0 静态锁的
    //   字面 `meta.round` 正则命不中 ⇒ 漏网。故此处用「专属静态锁 + 行为门禁 + 负向自证」三重锁。
    const srcCc2390 = fs.readFileSync(path.join(BASE, "engines/chatcache.js"), "utf8");
    const faceCc2390 = inv2390.codeFace(srcCc2390);
    const nestedGhost = function (face) { return (face.match(/meta\s*\|\|\s*\{\}\s*\)\s*\.\s*round/g) || []).length; };
    assert(nestedGhost(faceCc2390) === 0,
      "v2390: chatcache 真代码面无 `meta||{}).round` 幽灵读（实 " + nestedGhost(faceCc2390) + "）");
    assert(faceCc2390.indexOf("roundOfSafe(JSON.parse(getState(id)") >= 0,
      "v2390: chatcache 自动备份轮次已改走读口（静态锁）");
    W.backstage.setSettings({ autoBackup: true });
    assert(W.chatcache.runTick && typeof W.chatcache.runTick === "function", "v2390: chatcache.runTick 可驱");
    const autoBefore2390 = (W.chatcache.listSnapshots() || []).filter(function (s) { return s.auto; }).length;
    //   注：chatcache 自动快照有 MAX_AUTO_BACKUPS=3 环形裁剪 ⇒ 不能用「份数增长」当判据（会被裁掉），
    //   改判「是否真产出过自动快照 + 名字里带不带真源轮次」——旧实现 round 恒 0 ⇒ 永远 0 份。
    const drive2390 = function (r) {
      W.store.transact(function (d) { d.evolution.round = r; });
      W.store.save();
      W.chatcache.runTick();
    };
    drive2390(100);   // 首次只记基线
    drive2390(101);   // 推进 ⇒ 应产自动快照
    const autos2390 = (W.chatcache.listSnapshots() || []).filter(function (s) { return s.auto; });
    assert(autos2390.length >= 1 && (autoBefore2390 === 0 || autos2390.length > 0),
      "v2390: 轮次推进真产自动快照（实 " + autos2390.length + " 份；旧实现 round 恒 0 ⇒ 恒 0 份，开关形同虚设）");
    assert(autos2390.some(function (s) { return String(s.name).indexOf("第101轮") >= 0; }),
      "v2390: 自动快照名含真源轮次（实 " + autos2390.map(function (s) { return s.name; }).join("/") + "）");
    // ── H. 负向自证（真源码破坏 → 副本上重跑同款判据）──
    const brokenCc2390 = srcCc2390
      .split("roundOfSafe(JSON.parse(getState(id) || '{}'))")
      .join("(JSON.parse(getState(id) || '{}').meta || {}).round || 0");
    assert(brokenCc2390 !== srcCc2390, "v2390: （负向自证）chatcache 破坏锚点恰命中");
    assert(nestedGhost(inv2390.codeFace(brokenCc2390)) === 1,
      "v2390: （负向自证）退回嵌套幽灵写法后判据真报 1 处（实 " + nestedGhost(inv2390.codeFace(brokenCc2390)) + "）");
    W.backstage.setSettings({ autoBackup: false });
    // 清理：不留冷却残留给后续段
    W.store.transact(function (d) { delete d.proactiveLastRound; });
    console.log("  ✓ v2390: 顶层 round 幽灵收口（写入口/冷却/诊断/自检四处同源）");
