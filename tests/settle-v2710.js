...
const A_EG_DUP = "if (list.filter(function (k) { return k && k.who === who; })[0]) { out = { ok: false, reason: 'exists', key: key, who: who }; return; }";
const A_EG_KNOWERS = "if (list.length >= settings().maxKnowers) { out = { ok: false, reason: 'knowers-full', key: key, who: who, cap: settings().maxKnowers }; return; }";
const A_EG_ROWS = "if (draft.enigma.rows.length >= settings().maxRows) { out = { ok: false, reason: 'rows-full', key: key }; return; }";
const A_TP_GEAR = "if (!GEAR_MAP[gid]) { noteFault('bad-gear'); return { ok: false, reason: 'bad-gear', got: id }; }";
const A_TP_SAMEGEAR = "if (from === gid) { out = { ok: false, reason: 'same-gear', gear: gid }; return; }";
...
const A_QT_POOLFULL = "if (liveCount >= cap) { out = { ok: false, reason: 'pool-full', pool: p, cap: cap }; return; }";
const A_QT_TERMINAL = "if (hit.status === 'resolved' || hit.status === 'dropped') { out = { ok: false, reason: 'already-terminal', id: key, status: hit.status }; return; }";
...
const A_SL_PENDFULL = "if (draft.spotlight.pending.length >= pendCap) { out = { ok: false, reason: 'pending-full', who: w, cap: pendCap }; return; }";
const A_SL_EMPTY = "if (!pend.length) { out = { ok: false, reason: 'empty-round' }; return; }";
