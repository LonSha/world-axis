...
(function () {
  ...
  function defaultWorldState() {
    return {
      ...
      //   v2.100.0（第五十七面）对位：本键**一个字段都没加**。对位面全部是**读**——
      //   它读这份骨架的题名、读世界侧历史（chronicle / currents / echoes / chapters），
      //   把读数与证据**返回给调用方**，不往存档里写任何东西（「你演到哪一幕了」是问你，
      //   不是记你）。故本骨架仍是 `{ outline }` 一项；对位结果也不进存档——
      //   理由是它会随世界侧历史变化而变化，落盘等于把一份**过期读数**固化下来。
      canon: { outline: null },
