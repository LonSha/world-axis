# -*- coding: utf-8 -*-
# 生成 tests/negative-control-audit-v2104.js
# 占位约定：@@B@@ = 一个反斜杠字符；@@NL@@ = 反斜杠 + 字面 n（JS 源里的转义序列）
import io

BODY = r'''// WorldAxis tests/negative-control-audit-v2104.js (v2.104.0, 计划一 #2) — 负控制审计专锁
//
// 四段结构（与仓库既有专锁同规）：
...
const multi = 'line1@@NL@@line2';
const escaped = multi.replace(/@@NL@@/g, BS + 'n');
...
BODY = BODY.replace('@@B@@', chr(92)).replace('@@NL@@', chr(92) + 'n')
io.open('/tmp/wa_git/tests/negative-control-audit-v2104.js', 'w', encoding='utf-8').write(BODY)
print('WROTE tests/negative-control-audit-v2104.js lines=%d' % BODY.count('\n'))